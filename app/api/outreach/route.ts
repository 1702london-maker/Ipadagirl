...
export async function POST(req: NextRequest) {
  ...
  if (process.env.RESEND_API_KEY) {
    ...
    await resend.emails.send({
      ...
      subject: `New Outreach Request: ${school_name}, ${state}`,
      html: `<p><b>School:</b> ${school_name} (${school_type})</p><p><b>Contact:</b> ${contact_person} — ${position}</p><p><b>Email:</b> ${email}</p><p><b>Phone:</b> ${phone}</p><p><b>State:</b> ${state}, ${lga}</p><p><b>Address:</b> ${address}</p><p><b>Girls:</b> ${estimated_girls} (${age_range})</p><p><b>Period:</b> ${preferred_period}</p><p><b>Reason:</b> ${reason}</p>`,
    });
  }
